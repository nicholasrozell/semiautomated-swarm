...
class PathPlanning:
    def __init__(self):
        rospy.loginfo_once('INITIALIZING')
    ...
    def global_position(self, data):
        ...
        if self.gps_status >= -1: # needs to be >=0
    ...
    def main(self):
        ...
        rospy.Subscriber('/mavros/global_position/compass_hdg', Float64, self.global_heading)
        rospy.Subscriber('/mavros/global_position/global', NavSatFix, self.global_position)
        rospy.Subscriber('/mavros/global_position/local', Odometry, self.local_position)
        ...
        while not rospy.is_shutdown():
            ...
            if graph.num_nodes() <= 250:
                ...
                for i in range(len(pathNED)):
                    ...
                    wp_point.x_lat = pathNED[i][0]#pathNED[0][i]
                    wp_point.y_long = pathNED[i][1]#pathNED[1][i]
